...
use crate::{
    config::Config, scan::bitcoin::process_block_with_predicates, try_error, try_warn, utils::monitoring::PrometheusMonitoring
};
...
pub fn update_observer_progress(
    ...
    while let Err(e) = observers_db_conn.execute(
        ...
        try_warn!(ctx, "unable to query observers.sqlite: {}", e.to_string());
        std::thread::sleep(std::time::Duration::from_secs(1));
    }
}
